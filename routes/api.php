...
// 4-. Obtener todas las materias 
Route::get('subjects', function () {
    $subjects = Subject::all(['id','name_subject']);
    return response()->json($subjects);
});
//5-. Registrar materia
Route::post('subjects', function (Request $requets){
    try {
    $subject = new Subject();
    $subject->name_subject=$request->name_subject;
    $user->save();
    return response()->json([
        "message" => 'La materia a sido registrada exitosamente',
        'successful' => true
    ]);
    } catch (QueryException $exc) {
    return response()->json([
        "message" => 'La materia ya existe, introduzca uno diferente',
        'successful' => false
    ]);
    }
});
//6-Eliminar materia
Route::delete('subject/{subject_id}',function($subject_id){
    $subject=Subject::find($subject_id);
    $result=$subject->delete();
    if($result){
        return response()->json([
            "message" => 'La materia ha sido eliminado exitosamente',
            'successful' => true
        ]);
    }
    else
    {
        return response()->json([
            "message" => 'la operacion ha fallado, la materia no existe.',
            'successful' => false
        ]);
    }
});
//7-Obtener todos los grupos()
Route::get('groups', function () {
        $sub = SubjectUser::all();
        return array_values($sub->map(function($elem){
            $subject_name = Subject::find($elem->subject_id)->name_subject;
            $user_name = User::find($elem->user_id)->name;
            return array(
                 "id" => $elem->id,
...
//9 eliminar grupos
Route::delete('group/{subjectuser_id}', function ($subjectuser_id) {
    $group =SubjectUser::find($subjectuser_id);
    $result=$group->delete();
    if($result){
        return response()->json([
    ...
    {
        return response()->json([
            "message" => 'la operacion ha fallado, el grupo no existe.',
            'successful' => false
        ]);
    }
});
